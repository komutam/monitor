			...
			<c:if test="${login.userid == memberVO.userid}">
				...
				<div class="center_Post2">
					<div class="postBox2">
						...
						<div class="centerBar">
							<input type="file" class="file5" id="file5" onchange="fileInfo5(this)"/>
...
{{#if files}}
<li class="imgPostLi" data-pbno={{pbno}} data-id={{puserid}} data-date={{regdate}}>
...
$(function(){//모달 이벤트 처리
	$(".postList").on("click", ".postLi", function(event){
		...
		$(".modal-good").html(post.attr("data-good"));
	});
...
$(function(){//글 삭제 ajax 스크립트
	$("#postDeleteBtn").click(function(){
		...
		$.ajax({
			type : 'DELETE',
			url : '/monitor/account/delete/'+pbno,
			headers: { 
			      "Content-Type": "application/json",
...
$(function(){
	$(document).on('click','.heart',function(){//클릭 스크립트 실행 안될경우 이줄 사용
		...
		/* alert("a : "+pbno+", userid : "+userid+", heart : "+heart);	 */
  		...
  		$.ajax({
			type: 'POST',
			url: '/monitor/account/heart/'+pbno,
			headers: { 
			      "Content-Type": "application/json",
			      "X-HTTP-Method-Override": "POST" },
